...
void ayar() {
     TRISB = 0X31; //harici kesme ve DIP switch baðlantýlarý giriþ yapýlýyor
     PORTB = 0;
     TRISA = 0X00;
     PORTA = 0;
     ...
     CMCON=0X07;     //Analog karþýlaþtýrýcýlar kapatýldý
     OPTION_REG = 0b01000111;    // Önölçekleyici WDT olarak 1:128
     TMR1H=0X00;  //Süre = Prescaler*(65635-0+1)*_____1_____ = 8*2 = 16sn
     TMR1L=0X00;  //                                32768
     Delay_us(10);

     T1CON.TMR1ON=0; //Timer1 önce kapalý
     T1CON=0X3F;     //Ön ölçekleyici 1:8
     Sound_Init(&PORTB, 3);  //Buzzer'ýn baðlý oluðu port
     MAVI=0;
     KIRMIZI=0;
     SES=0;
     ikaz_biti=0;
...
void Alarm() {
     Sound_Play(1000, 50);   // 1kHz-50ms fasýlý sinyal
}
void bitti() {
     PORTB=0X00;
     zaman=0;
...
void interrupt(){
 ...
 TMR1H=0X00; //Timer1 yüksek baytý yükleniyor
 T1CON.TMR1ON=0; // Timer1 kapatýlýyor
 T1CON=0x3F; // Süre = Prescaler*(65536-0)*___1___=8*2=16sn
             //                             32768
 PIR1.TMR1IF=0;
 }
 }